...
void *thread_main(void *rm)
{
    ...
    int sockid;
    int port = SOCKETPORT + room_count;
    room.client_count = room_count;
    ...
    while((client.disc = accept(sockid, (struct sockaddr *)&csockaddress, &clen)))
    {
        ...
        client.num = sockid;
        room.num = sockid;
        room.client_list[room.client_count] == client.disc;
        
        rlist[room.client_count] = room;
...
int main()
{
    ...
    for(i=0;i<room;i++)
    {
        pthread_create(&ptr[i], NULL, thread_main, &room);
